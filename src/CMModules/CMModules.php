        ...
        while (($module = $dir->read()) !== false) {
          if(is_dir("$src/$module")) {
            if(class_exists($module)) {
              ...
              $modules[$module]['isRidcullyCore']   = in_array($rc->name, array('CRidcully', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
              $modules[$module]['isRidcullyCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
            }
          }
        }
        $dir->close();
