<?php
/**
 * A model for managing Ridcully modules.
 *
...
class CMModules extends CObject {
   ...
   * Properties
   */
  private $ridcullyCoreModules = array('CRidcully', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $ridcullyCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
    ...
    while (($module = $dir->read()) !== false) {
      if(is_dir("$src/$module")) {
        if(class_exists($module)) {
          ...
          $modules[$module]['isRidcullyCore'] = in_array($rc->name, $this->ridcullyCoreModules);
          $modules[$module]['isRidcullyCMF']  = in_array($rc->name, $this->ridcullyCMFModules);
        }
      }
    }
    $dir->close();
   ...
   *
   * @param string $module string with the module name.
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isRidcullyCore']   = in_array($rc->name, $this->ridcullyCoreModules);
      $details['isRidcullyCMF']    = in_array($rc->name, $this->ridcullyCMFModules);
