...
class CMContent extends CObject implements IHasSQL, ArrayAccess {
    ...
    public static function SQL($key=null) {
        ...
        $queries = array(
            ...
            'select * by id'          => 'SELECT c.*, u.acronym as owner FROM r_content AS c INNER JOIN r_user as u ON c.idUser=u.id WHERE c.id=?;',
            'select * by key'         => 'SELECT c.*, u.acronym as owner FROM r_content AS c INNER JOIN r_user as u ON c.idUser=u.id WHERE c.key=?;',
            'select * by type'        => "SELECT c.*, u.acronym as owner FROM r_content AS c INNER JOIN r_user as u ON c.idUser=u.id WHERE type=? ORDER BY {$order_by} {$order_order};",
            'select *'                => 'SELECT c.*, u.acronym as owner FROM r_content AS c INNER JOIN r_user as u ON c.idUser=u.id;',
            'update content'          => "UPDATE r_content SET key=?, type=?, title=?, data=?, filter=?, updated=datetime('now') WHERE id=?;",
        );
